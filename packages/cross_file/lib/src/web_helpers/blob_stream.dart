...
/// This class streams an [web.Blob] in chunks of [MAX_CHUNK_SIZE] bytes.
class BlobStream extends Stream<Uint8List> {
  ...
  BlobStream(Future<web.Blob> blob, [int? start, int? end])
      : _blob = blob,
        _nextByte = start ?? 0,
        _finalByte = end;

  // The source of data that we want to Stream
  final Future<web.Blob> _blob;
  ...
  Future<void> _readChunk() async {
    ...
    return _blob
        .then((web.Blob blob) => blob.slice(_nextByte, _nextByte + chunkSize))
